namespace WHICHEATSERVER.Core.Serialization
{
    using WHICHEATSERVER.Core.Utilits;
    using WHICHEATSERVER.Core.ValueType;
    using System;
    ...
    public sealed partial class AdvancedObjectFormatter
    {
        ...
        private static int GetSpecificSize(AdvancedObjectMarshalAsAttribute attr, Type clazz)
        {
            ...
            return ValueTypeUnit.SizeBy(clazz);
        }
        ...
        {
            if (TypeUnit.GetArrayElement(clazz) == null)
            {
    ...
    public sealed partial class AdvancedObjectFormatter
    {
        ...
        private static object GetValueFromStream(Type clazz, AdvancedObjectMarshalAsAttribute attr, BinaryReader reader)
        {
            int size = ValueTypeUnit.SizeBy(clazz);
            if (size > 0)
                return ValueTypeUnit.LongTo(clazz, ValueTypeUnit.ToLong(clazz, AdvancedObjectFormatter.As(reader.ReadBytes(size), attr), size));
            else
            {
                if (TypeUnit.IsString(clazz))
                    return AdvancedObjectFormatter.GetStringFromStream(attr, reader);
                else if (TypeUnit.GetArrayElement(clazz) != null)
